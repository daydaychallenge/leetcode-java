...
public class RandomizedSet {
    ...
    public static void main(String[] args) {
        ...
        r.insert(2); // Inserts 2 to the set, returns true. Set now contains [1,2].
    ...
    public boolean remove(int val) {
        // 得到当前的需要删除的值的下标，然后判断是否是 list 的最后一个，如果不是，则进行下标置换，然后移除最后一个。
        if (cache.containsKey(val)) {
            int index = cache.remove(val);
            if (index < values.size() - 1) {
                // 置换
                int latestVal = values.get(values.size() - 1);
                cache.put(latestVal, index);
                values.set(index, latestVal);
            }
